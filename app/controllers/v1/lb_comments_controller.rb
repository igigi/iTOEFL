class V1::LbCommentsController < ApplicationController
  before_action :set_lb_comment, only: [:show, :edit, :update, :destroy]
  before_action :set_live_broadcast, only: [:create, :index]
  ...
  def create
    @lb_comment = @live_broadcast.lb_comments.build(lb_comment_params)
  ...
  private
    ...
    def set_live_broadcast
      @live_broadcast = LiveBroadcast.find(params[:live_broadcast_id])
    end
